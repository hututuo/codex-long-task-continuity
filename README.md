...
1. 压缩连续性、当前有效的子 Agent 授权与 `Completion gate`，让压缩摘要明确保留任务如何继续以及为什么还不能结束。
...
| 多次压缩后忘记用户是否允许使用子 Agent | 摘要显式记录用户当前有效的子 Agent 授权和相关要求或限制，后续要求覆盖先前要求 |
...
| `$CODEX_HOME/prompt-overlays/compaction-continuity.md` | 压缩连续性、子 Agent 授权状态与 Completion gate |
...
6. Conclusion state
7. Unresolved leads
8. Open-conclusion state
9. Remaining work
10. Communication and delivery state
11. Completion gate

其中 `Delegation authorization` 明确保留用户当前有效的子 Agent 使用授权及其要求或限制，后续要求覆盖先前要求。`Completion gate` 是结束判定：必须写明最终回复前尚需满足的条件，以及当前任务为什么不能结束。只要还有必要条件未满足，任务状态就必须保持为进行中或受阻，并与剩余工作一致。
